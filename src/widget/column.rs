    ...
    Theme: Catalog,
{
    ...
    /// Sets the [`Id`](position::Id) of the [`Column`] for position tracking.
    ///
    /// When set, the [`Column`] tracks the layout bounds of each child widget.
...
///
/// For Column: primary axis is Y (top-to-bottom), cross axis is X (wrapping into columns).
...
fn compute_reorder_offsets(
    ...
    // First pass: simulate wrapping layout, track column boundaries
    ...
    for (seq, &idx) in order.iter().enumerate() {
        ...
        if y > 0.0 && y + h > wrap_extent {
            cols.push(ColInfo {
                start: col_start,
                end: seq,
                width: col_width,
            });
            x += col_width + cross_spacing;
            y = 0.0;
            col_width = 0.0;
            col_start = seq;
        }
    ...
    // Second pass: apply cross-axis alignment (horizontal for Column)
    ...
    if align_factor != 0.0 {
        for col_info in &cols {
            for &idx in &order[col_info.start..col_info.end] {
                let w = child_bounds[idx].width;
                simulated_pos[idx].x += (col_info.width - w) / align_factor;
            }
        }
    }
    ...
    Theme: Catalog,
{
    ...
    fn operate(
        ...
        operation.container(id, layout.bounds(), viewport);
    ...
    Theme: Catalog,
{
    ...
    fn layout(
        ...
        match action {
            ...
            | Action::Picking { animations, .. }
            | Action::Dragging { animations, .. } => {
                animations.wrap_extent = max_height;
                animations.cross_spacing = horizontal_spacing;
            }
        }
