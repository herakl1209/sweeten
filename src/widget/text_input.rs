        ...
        if let Some(on_input) = &self.on_input {
            ...
            let edit = state.input.update(
                ...
                |key_press| {
                    ...
                    {
                        return Some(editor::Binding::Custom(
                            on_submit.clone(),
                        ));
                    }
            ...
            if let Some(edit) = edit {
                ...
                state.transaction = Some(
                    shell.publish_and_track(on_input(state.value.clone())),
                );
            }
        }
...
fn state<Renderer: text::Renderer + 'static>(
    tree: &mut Tree,
) -> &mut State<Renderer> {
